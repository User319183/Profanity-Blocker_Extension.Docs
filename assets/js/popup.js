	...
	chrome.storage.local.get(['password'],function(pass){
		chrome.storage.local.get(['filterToggle'],function(toggle){
			...
			if(passwordProperty ===  "null"){
				...
				var html = '<h4> Please Enter Your New Password </h4>';
				...
				var html = '<h4> Please Enter Your Password </h4>';
...
function optionPassword(event){
	...
	chrome.storage.local.get(['password'],function(pass){
		if(optionPass === pass.password){
			alert("Password successfully entered! Redirecting to configurations.")
			console.log("Entering configurations")
			chrome.runtime.openOptionsPage(); 
		}

		else{
			var html = "Incorrect Password, please try again.";
...
document.getElementById('options').addEventListener('click', function() {
	...
	chrome.storage.local.get(['password'],function(pass){
		if(pass.password === "null"){
			html = '<h4> ERROR: Please click the toggle and setup a password. </h4>';
			document.getElementById('optionsPassword').innerHTML = html;
		}

		else{
			html = '<h4> Please Enter your Password. </h4>';
